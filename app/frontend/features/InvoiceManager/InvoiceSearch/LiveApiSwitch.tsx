...
const LiveApiSwitch: FC<{ disabled?: boolean }> = ({ disabled }) => {
  const toggleClassNames = clsx('inline-flex items-center space-x-2', !disabled && 'cursor-pointer', disabled && 'cursor-not-allowed')

  return (
    <label className={toggleClassNames}>
      <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">Live API</span>
      <input type="checkbox" value="" className="sr-only peer" disabled={disabled} />
