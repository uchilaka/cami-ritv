...
export default function ThDueDate() {
  ...
  const sortDirection = searchParams?.s?.dueAt
  ...
  const toggleSortFilter = useCallback(() => {
    if (sortingApplied && sortingDesc) {
      updateSearchParams({ s: { dueAt: 'asc' } })
    } else {
      updateSearchParams({ s: { dueAt: 'desc' } })
    }
  ...
  return (
    <th scope="col" className="px-6 py-3">
      {isEnabled('sortable_invoice_index', 'invoice_sortable_by_due_date') ? (
        <>
          <a ref={controlRef} href="#" className="text-blue-600 dark:text-blue-500 hover:underline" onClick={toggleSortFilter}>
            {labelText}
            <i
              className={clsx(
                'fa-solid px-2',
